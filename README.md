...
For more details, see [docs/ARCHITECTURE.md](file:///docs/ARCHITECTURE.md).
...
For more details, see [docs/RETRIEVAL_METHOD.md](file:///docs/RETRIEVAL_METHOD.md).
...
When the evidence threshold is passed, the LLM (`gpt-3.5-turbo`) is supplied with:
...
The temperature is set to `0.1` to force factual compliance.
...
Real outputs generated by querying the actual running system are saved in [examples/cited_answers.md](file:///examples/cited_answers.md).
...
pytest tests/ -v
```
All **21 test cases** pass successfully. For detailed descriptions of the tests, see [docs/TESTING.md](file:///docs/TESTING.md).

---

## Tradeoffs

A detailed outline of system design tradeoffs (local vs hosted, hybrid vs semantic, SQLite vs vector DBs) is documented in [docs/TRADEOFFS.md](file:///docs/TRADEOFFS.md).
